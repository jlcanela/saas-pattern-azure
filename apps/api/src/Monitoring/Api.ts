...
import { ActionTag, Authorization } from "../Middleware/Authorization.js";
...
export const MonitoringApi = HttpApiGroup.make("monitoring")
  ...
  .add(HttpApiEndpoint.post("auth")`/auth`.setPayload(AuthRequest).addSuccess(AuthorizationResult).annotate(ActionTag, { action: "ping" })) 
